...
public class ProgramLoader {
    ...
    private ObservableList<IStatement> getPrograms(){
        ...
        IStatement ex11 = new CompStatement(new VarDeclaration("v", IntType.T), new CompStatement(new VarDeclaration("w", IntType.T), new CompStatement(new Assignment("v", new ValueExpression(new IntValue(2))), new CompStatement(new Assignment("w", new ValueExpression(new IntValue(5))), new CompStatement(new CallProcedureStatement("sum", Arrays.asList(new ArithmeticExpression(new VariableExpression("v"),new ValueExpression(new IntValue(10)), '*'), new VariableExpression("w"))), new CompStatement(new PrintStatement(new VariableExpression("v")), new CompStatement(new ForkStatement(new CallProcedureStatement("product", Arrays.asList(new VariableExpression("v"), new VariableExpression("w")))), new ForkStatement(new CallProcedureStatement("sum", Arrays.asList(new VariableExpression("v"), new VariableExpression("w")))))))))));
    ...
    private Controller initializeProgram(IStatement program){
        ...
        IStack<IDictionary<String, IValue>> symTableStack = new MyStack<>();
        symTableStack.push(dic);
        IList<IValue> out = new MyList<>();
        IDictionary<StringValue, BufferedReader> fileTable = new MyDictionary<>();
        IHeap<IValue> heap = new MyHeap<>();
        IDictionary<String, Pair<List<String>, IStatement>> procTable = new MyDictionary<>();
        procTable.add("sum", new Pair<>(Arrays.asList("a", "b"), new CompStatement(new VarDeclaration("v", IntType.T), new CompStatement(new Assignment("v", new ArithmeticExpression(new VariableExpression("a"), new VariableExpression("b"), '+')), new PrintStatement(new VariableExpression("v"))))));
        procTable.add("product", new Pair<>(Arrays.asList("a", "b"), new CompStatement(new VarDeclaration("v", IntType.T), new CompStatement(new Assignment("v", new ArithmeticExpression(new VariableExpression("a"), new VariableExpression("b"), '*')), new PrintStatement(new VariableExpression("v"))))));
        ...
        ProgramState state = new ProgramState(stack, symTableStack, out, program, fileTable, heap, procTable);
        try {
