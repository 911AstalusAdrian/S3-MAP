...
public class ProgramRunner implements Observer {
    ...
    @FXML
    private TableColumn<Map.Entry<String, Pair<List<String>, IStatement>>, String> procName;

    @FXML
    private TableColumn<Map.Entry<String, Pair<List<String>, IStatement>>, String> procFormalParams;

    @FXML
    private TableColumn<Map.Entry<String, Pair<List<String>, IStatement>>, String> procBody;
    ...
    private void setSymbolTableAndExecutionStack() {
        ...
        if (result != null){
            ...
            symbolTableList.addAll(result.getTopSymTable().getContent().entrySet());
    ...
    private void setProcTable(){
        ObservableList<Map.Entry<String, Pair<List<String>, IStatement>>> procTableList = FXCollections.observableArrayList();
        procName.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getKey()));
        procFormalParams.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getValue().getKey().toString()));
        procBody.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getValue().getValue().toString()));
        procTableList.addAll(cont.getRepository().getProgramList().get(0).getProcTable().getContent().entrySet());
