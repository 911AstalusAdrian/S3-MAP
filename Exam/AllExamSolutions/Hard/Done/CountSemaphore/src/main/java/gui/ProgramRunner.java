...
public class ProgramRunner implements Observer {
    ...
    @FXML
    private TableColumn<Map.Entry<Integer, Pair<Integer, List<Integer>>>, String> semaphoreAddress;

    @FXML
    private TableColumn<Map.Entry<Integer, Pair<Integer, List<Integer>>>, String> semaphoreCount;

    @FXML
    private TableColumn<Map.Entry<Integer, Pair<Integer, List<Integer>>>, String> semaphoreIDs;
    ...
    private void setSemaphoreTable(){
        ...
        semaphoreAddress.setCellValueFactory(cellData -> new SimpleStringProperty(Integer.toString(cellData.getValue().getKey())));
        semaphoreCount.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getValue().getKey().toString()));
        semaphoreIDs.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getValue().getValue().toString()));
        semaphoreTableList.addAll(cont.getRepository().getProgramList().get(0).getSemaphoreTable().getContent().entrySet());
