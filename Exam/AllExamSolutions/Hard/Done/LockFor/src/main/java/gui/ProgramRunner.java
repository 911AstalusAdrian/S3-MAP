...
public class ProgramRunner implements Observer {
    ...
    @FXML
    private TableColumn<Map.Entry<Integer, Integer>, String> lockTableAddr;

    @FXML
    private TableColumn<Map.Entry<Integer, Integer>, String> lockOwnerID;
    ...
    private void setLockTable() {
        ...
        lockTableAddr.setCellValueFactory(cellData -> new SimpleStringProperty(Integer.toString(cellData.getValue().getKey())));
        lockOwnerID.setCellValueFactory(cellData -> new SimpleStringProperty(Integer.toString(cellData.getValue().getValue())));
        lockTableList.addAll(cont.getRepository().getProgramList().get(0).getLockTable().getContent().entrySet());
