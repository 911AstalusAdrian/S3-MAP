...
public class ProgramRunner implements Observer {
    ...
    @FXML
    private TableColumn<Map.Entry<Integer, Pair<Integer, List<Integer>>>, String> barrierAddress;

    @FXML
    private TableColumn<Map.Entry<Integer, Pair<Integer, List<Integer>>>, String> barrierCount;

    @FXML
    private TableColumn<Map.Entry<Integer, Pair<Integer, List<Integer>>>, String> barrierIDs;
    ...
    private void setBarrierTable(){
        ...
        barrierAddress.setCellValueFactory(cellData -> new SimpleStringProperty(Integer.toString(cellData.getValue().getKey())));
        barrierCount.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getValue().getKey().toString()));
        barrierIDs.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getValue().toString()));
        barrierTableList.addAll(cont.getRepository().getProgramList().get(0).getBarrierTable().getContent().entrySet());
